...
describe("RevshareVault", function() {
    ...
    const simulateTrade = async({
        ...
        const {dxblToken, revshareVault, wallets} = props;
        ...
        const cBal = +inDecs(await usdcCon.balanceOf(revshareVault.address), 6);
        console.log("Setting USDC balance for vault starting at", cBal);
        await setUSDCBalance({
            ethers,
            provider: ethers.provider,
            chain: NET,
            tgtAddress: revshareVault.address,
            balance: cBal + (usdcAmount*(4/10000))
        });

        console.log("Rewarding trader with volume: " + usdcAmount);
        let txn = await revshareVault.connect(wallets.dexibleAdmin).rewardTrader(trader.address, feeToken.token, inAmount);
        ...
        if(burnPerc) {
            ...
            const estOut = await props.revshareVault.estimateRedemption(feeTokens[NET][0].token, burnAmt);
        
            console.log("Will burn", inDecs(burnAmt, 18), `(${burnPerc}%) for`, estOut);
            txn = await revshareVault.connect(trader).redeemDXBL(feeTokens[NET][0].token, burnAmt, bn(estOut).mul(999).div(10000));
            r = await txn.wait();
    ...
    it("Should mint tokens to trader", async () => {
        ...
        const rate = await revshareVault.currentMintRateUSD();
    ...
    it("Should mint tokens at lower rate once exceeded bounds", async () => {
       ...
       await simulateTrade({
        ...
        const mintRate = +(inDecs(await revshareVault.currentMintRateUSD(), USD_PRECISION));
    ...
    it("Should redeem DXBL for USDC", async () => {
        ...
        const trader = wallets.all[2];
        const nav = await revshareVault.currentNavUSD();
        if(nav.eq(0)) {
        ...
        const burn = 1000;
        const estOut = await props.revshareVault.estimateRedemption(feeTokens[NET][0].token, inUnits(bal.toString(), 18));
        console.log("Est out", estOut);
        
        await revshareVault.connect(trader).redeemDXBL(asUSDCToken(NET).address, inUnits(burn.toString(), 18), bn(estOut).mul(999).div(10000));
        ...
        console.log("Assets", await revshareVault.assets());
    });
    

    const showState = async (prefix) => {
        const {revshareVault: vault, dxblToken, wallets} = props;
        const trader = wallets.all[2];
    ...
    it("Should roll over 24hr volume correctly", async () => {
        const {revshareVault,dxblToken, wallets} = props;
        const trader = wallets.all[2];
        ...
        const txn = await revshareVault.connect(trader).redeemDXBL(feeTokens[NET][0].token, burnAmt, bn(estOut).mul(999).div(10000));
